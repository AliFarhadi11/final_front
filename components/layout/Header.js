...
export default function Header() {
  ...
  return (
    <>
      <header
        className={`${
          router.pathname == "/learn-more" ? "header mb-0 position-relative" : "header position-relative"
        }`}>
        <nav className="header-nav d-flex justify-content-between align-items-center">
          {/* BrandName */}
          <Link href="/" className="brandname">
            <Image src={text_logo} width="147" height="auto" alt="playersCube" className="d-lg-block d-none" />
            <Image src={logo} width="40" height="auto" alt="playersCube" className="d-lg-none d-block" />
          </Link>
          ...
          <div className="nav-right-links d-flex align-items-center d-lg-flex d-none">
            ...
            {!isEmpty(user) && (
              <>
                <Grid2 item container justifyContent="flex-end" alignItems="center" sx={{ mx: 0 }}>
                  <UserMenu user={user} />
                </Grid2>
              </>
            )}
          ...
          {isEmpty(user) && (
            <div className="d-block d-lg-none cursor-pointer" onClick={() => setIsToggled((prev) => !prev)}>

...
import { Chart as ChartJs, LineElement, PointElement, Tooltip, RadialLinearScale, Filler } from "chart.js";
...
function PlayerReport() {
  ...
  const marketValue = (MarketValue) => {
    if (MarketValue < 999999) return "€" + (MarketValue / 1000).toFixed(0) + "k";
    if (MarketValue > 999999) return "€" + (MarketValue / 1000000).toFixed(2) + "m";
  };
  const handleGetPlayerInfo = async (event) => {
    try {
      ...
      if (status === 200) {
        ...
        list.forEach((ses) => {
          data.details.forEach((item) => {
            if (item.Season === ses) rating_hist.push(item.rating);
          });
        });
        setRatingsData({
          labels: list,
        ...
        setSeasonList(list);
        setHistory(hist);
        if (router.query.season == 0) setSeason(list[list.length - 1]);
        else setSeason(router.query.season);
      }
    } catch (ex) {}
  };
  ...
  useEffect(() => {
    if (router.query.id) handleGetPlayerInfo();
  }, [router.query]);
  ...
  return (
    <>
      <Head>
        ...
        <title> {contorlers.lang === "fa" ? "انتخاب بازیکنان برتر فوتبال" : "Soccer Player Report"} </title>
        <meta
          name="description"
          content={
            contorlers.lang === "fa" ? "انتخاب بازیکنان برتر فوتبال" : "NEW ERA TO FIND THE BEST PLAYER"
          }
        />
      ...
      {!isEmpty(player_report) && player_report.details.length > 0 ? (
        <main className="player-system-container container-fluid">
          ...
          <div className="player-report">
                  ...
                  <div className="info-wrapper">
                    <div>
                      ...
                      <div className="d-flex justify-content-between align-items-center mt-4">
                        {!isEmpty(player_report.player.Instagram) && player_report.player.Instagram != " " && (
                          <Link target="_blank" rel="noreferrer" href={player_report.player.Instagram}>
                            <Image src={instagram_outline_icon} width="auto" height="auto" alt="Instagram" />
                          </Link>
                        )}
                        {!isEmpty(player_report.player.Twitter) && player_report.player.Twitter != " " && (
                          <Link
                            target="_blank"
                            rel="noreferrer"
                            href={"https://www.twitter.com/" + player_report.player.Twitter}>
                            <Image src={twitter_outline_icon} width="auto" height="auto" alt="twiiter" />
                          </Link>
                        )}
                        {!isEmpty(player_report.player.Facebook) && player_report.player.Facebook != " " && (
                          <Link
                            target="_blank"
                            rel="noreferrer"
                            href={"https://www.facebook.com/" + player_report.player.Twitter}>
                            <Image src={twitter_outline_icon} width="auto" height="auto" alt="facebook" />
                          </Link>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="row mb-4">
                        <span className="cold-md-7 col text-capitilize">Date Of Birth:</span>
                        <span className="col font-gilroy-semibold text-end text-md-start">
                      ...
                      {/* Info */}
                      <div className="row mb-4">
                        <span className="cold-md-7 col text-capitilize">Main Position:</span>
                        <span className="col font-gilroy-semibold text-end text-md-start">
                          {detail.MainPositon}
                        </span>
                      </div>
                      ...
                      <div className="row mb-4">
                        ...
                        <span className="col font-gilroy-semibold text-end text-md-start"> {detail.Club}</span>
                      </div>
                      {/* Info */}
                      <div className="row mb-4">
                        <span className="cold-md-7 col text-capitilize">Nation Ply/Gls:</span>
                        <span className="col font-gilroy-semibold text-end text-md-start">
                          {" "}
                      ...
                      {/* Info */}
                      <div className="row mb-4">
                        <span className="cold-md-7 col text-capitilize">Current Market value:</span>
                        <span className="col font-gilroy-semibold text-end text-md-start">
                          {" "}
                      ...
                      <div className="row mb-4">
                        ...
                        <span className="col font-gilroy-semibold text-end text-md-start">{detail.Agent}</span>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="row gap-lg-0 gap-4 mb-3 mt-4" id="transfer-history">
              ...
              <div className="col-12 col-lg-6" id="video">
                <div className="report-wrapper video-highlights">
                  ...
                  <div className="video-wrapper">
                    ...
                    <div className="thumbnail" style={{ display: `${isPlay} ` ? "none" : "flex" }}>
                      <h5>Video Title</h5>
